...
<nav class="public-navbar">
    ...
    <div class="public-nav-links">
        ...
        <a href="<?= ADMIN_URL ?>/login.php" class="btn btn-primary" style="color:#fff; padding:0.4rem 1rem;">Admin Login</a>
    </div>
</nav>
...
<div class="center-card-wrapper">
    <div class="selector-card">
        ...
        <button class="btn btn-success compare-action-btn" id="compareBtn" disabled onclick="executeCompare()">
            Compare Now 
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="margin-left:4px;"><polyline points="5 12 19 12"/><polyline points="12 5 19 12 12 19"/></svg>
        </button>
    </div>
</div>
...
<div class="uni-modal-bg" id="uniModalBg">
    <div class="uni-modal">
        ...
        <input type="text" class="search-input" id="uniSearch" placeholder="Search university by name..." oninput="filterUniversities()">
        <div class="search-results" id="uniList"></div>
    </div>
</div>
<!--  BROCHURE MODAL -->
<div class="uni-modal-bg" id="brochureModalBg" style="z-index: 9999;">
    <div class="uni-modal" style="max-width: 450px; padding: 2rem;">
        <div class="modal-header" style="justify-content: center; position: relative;">
            <h2 style="margin: 0; color: var(--accent-blue); text-align: center;">Download Brochure</h2>
            <button class="modal-close" style="position: absolute; right: 0;" onclick="closeBrochureModal()">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
            </button>
        </div>
        <p style="text-align: center; color: var(--text-m); margin-bottom: 1.5rem; font-weight: 500;">Academic Experts will assist you!</p>
        
        <form id="brochureForm" onsubmit="submitBrochureForm(event)">
            <div class="brochure-form-group">
                <input type="text" id="brof_name" placeholder="Enter Your Name" required>
            </div>
            <div class="brochure-form-group">
                <input type="email" id="brof_email" placeholder="Enter Your Email" required>
            </div>
            <div class="brochure-form-group" style="display: flex;">
                <select id="brof_country_code" style="width: 100px; margin-right: 10px; border-radius: var(--radius-sm); border: 1px solid var(--border); background: #f8fafc; font-size:0.9rem;" required>
                ...
                <input type="tel" id="brof_number" placeholder="Enter your Number" required pattern="[0-9]{8,15}" title="8 to 15 digit mobile number" style="flex:1;">
            </div>
            <div class="brochure-form-group">
                <select id="brof_course" required>
                    <option value="">Select Your Course</option>
                    <!-- Populated dynamically via JS from master list -->
                </select>
            </div>
            <div class="brochure-form-group">
                <select id="brof_state" required>
            ...
            <div style="display: flex; align-items: flex-start; gap: 8px; margin-bottom: 1.5rem; font-size: 0.8rem; color: var(--text);">
                <input type="checkbox" id="brof_consent" required style="margin-top: 3px;">
                <label for="brof_consent">I consent to receive university updates via email and mobile number. <a href="#" style="color:var(--accent-blue);">Disclaimer</a></label>
            </div>
            
            <button type="submit" class="btn btn-success" id="brof_submit" style="width: 100%; border-radius: var(--radius-sm); font-size: 1.1rem; font-weight: 600; padding: 0.8rem; background: #10b981;">
                Download Brochure
            </button>
        </form>
    </div>
...
<script>
    ...
    function renderTable(courseName, unis) {
        ...
        rowsHtml += buildRow("SCHOLARSHIP", "gift", u => cellFn(u, x => {
             if(x.fees_discount > 0) {
                 return `<div class="desktop-scholarship">
                            ...
                            <button class="btn btn-success" style="padding:0.4rem 1rem; border-radius:30px; font-size:0.85rem; height:auto;">Claim Now</button>
                         </div>
                         <div class="mobile-scholarship">
                            <strong style="font-size:0.9rem;">Upto ₹${x.fees_discount}</strong>
                            <button class="btn btn-success" style="padding:0.4rem 1rem; border-radius:30px; font-size:0.8rem; height:auto;">Claim Now</button>
                         </div>`;
        ...
        rowsHtml += buildRow("GET FREE COUNSELING", "phone", u => cellFn(u, x => {
             return `<button class="btn btn-primary" style="background:#1b84ff; border-color:#1b84ff; border-radius:8px; display:flex; flex-direction:column; align-items:center; width:100%; max-width:140px; margin:0 auto; padding:0.5rem;">
                        ...
                        <span style="font-size:0.7rem; font-weight:700; color:#fff; text-transform:uppercase; margin-top:2px;">Book Free<br>Mentoring</span>
                     </button>`;
        }), unis);
    ...
    function openBrochureModal(url) {
        ...
        let cSelect = document.getElementById('brof_course');
    ...
    function submitBrochureForm(e) {
        ...
        let fBtn = document.getElementById('brof_submit');
        fBtn.innerText = 'Submitting...';
        fBtn.disabled = true;

        let fd = new FormData();
        fd.append('name', document.getElementById('brof_name').value);
        fd.append('email', document.getElementById('brof_email').value);
        fd.append('country_code', document.getElementById('brof_country_code').value);
        fd.append('phone', document.getElementById('brof_number').value);
        fd.append('course', document.getElementById('brof_course').value);
        fd.append('state', document.getElementById('brof_state').value);
        fd.append('page_url', window.location.href);

        fetch(BASE_URL + '/api/submit_lead.php', { method: 'POST', body: fd })
        .then(res => res.json())
        .then(json => {
            if(json.success && pendingBrochureUrl) {
                window.open(pendingBrochureUrl, '_blank');
            } else if (!json.success) {
                alert(json.error || 'Something went wrong while downloading.');
            }
        ...
        .finally(() => {
            fBtn.innerText = 'Download Brochure';
            fBtn.disabled = false;
        });
    }
</script>
