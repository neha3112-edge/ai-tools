...
$id = (int)($_GET['id'] ?? 0);
if (!$id) { set_flash('error','Invalid university.'); redirect(ADMIN_URL.'/universities/index.php'); }

$stmt = $pdo->prepare("SELECT * FROM universities WHERE id=? AND is_active=1");
$stmt->execute([$id]);
$uni = $stmt->fetch();
if (!$uni) { set_flash('error','University not found.'); redirect(ADMIN_URL.'/universities/index.php'); }
...
$sel_edu->execute([$id]); $cur_edu = array_column($sel_edu->fetchAll(),'education_mode_id');

$sel_exam = $pdo->prepare("SELECT exam_mode_id FROM university_exam_modes WHERE university_id=?");
$sel_exam->execute([$id]); $cur_exam = array_column($sel_exam->fetchAll(),'exam_mode_id');

$sel_accr = $pdo->prepare("SELECT accreditation_id FROM university_accreditations WHERE university_id=?");
$sel_accr->execute([$id]); $cur_accr = array_column($sel_accr->fetchAll(),'accreditation_id');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors) && !empty($_FILES['image']['name'])) {
        ...
        if ($uploaded) { delete_file($uni['image']); $image = $uploaded; }
        else $errors['image'] = 'Invalid image file.';
    }
    ...
    if (empty($errors) && !empty($_FILES['sample_certificate']['name'])) {
        ...
        if ($uploaded) { delete_file($uni['sample_certificate']); $cert = $uploaded; }
        else $errors['sample_certificate'] = 'Invalid certificate file.';
    }
    ...
    if (empty($errors)) {
        ...
        try {
            ...
            if (!empty($_POST['education_modes'])) {
                ...
                foreach ($_POST['education_modes'] as $mid) $ins->execute([$id,(int)$mid]);
            }
            ...
            if (!empty($_POST['exam_modes'])) {
                ...
                foreach ($_POST['exam_modes'] as $eid) $ins->execute([$id,(int)$eid]);
            }
            ...
            if (!empty($_POST['accreditations'])) {
                ...
                foreach ($_POST['accreditations'] as $aid) $ins->execute([$id,(int)$aid]);
            }
        ...
        <div class="form-grid">
          ...
          <div class="form-group">
            ...
            <input name="display_name" type="text" class="form-control"
                   placeholder="Leave blank to use main name" value="<?= e($old['display_name'] ?? '') ?>">
          </div>
          <div class="form-group">
            <label>Slug</label>
            <input id="uni_slug" name="slug" type="text" class="form-control"
                   value="<?= e($old['slug'] ?? '') ?>">
            <span class="form-hint">Preview: <code id="uni_slug_preview" style="color:var(--accent);"><?= e(get_slug($old['name'], $old['slug'] ?? null)) ?></code></span>
            <?php if (isset($errors['slug'])): ?><span class="form-hint" style="color:var(--danger)"><?= e($errors['slug']) ?></span><?php endif; ?>
            <span class="form-hint" style="color:var(--warning);">⚠ Changing slug may break existing links.</span>
          </div>
          <div class="form-group">
      ...
      <div class="form-card" style="margin-bottom:1.25rem;">
        <div class="form-group">
          ...
          <span class="form-hint" style="margin-bottom:8px;display:block;">
            Manage accreditation options in
          ...
          <?php else: ?>
            <p style="color:var(--text-s);font-size:13px;">No accreditations found.</p>
          <?php endif; ?>
        </div>
      </div>
      ...
      <div class="form-card" style="margin-bottom:1.25rem;">
        <div class="form-grid">
          <div class="form-group">
            ...
            <div style="display:flex;align-items:flex-start;gap:1rem;flex-wrap:wrap;">
              <div class="img-preview-wrap" id="img_preview_uni">
                <?php if ($uni['image']): ?>
                  <img src="<?= e($uni['image']) ?>" style="width:100%;height:100%;object-fit:cover;" alt="">
                <?php else: ?><span style="font-size:11px;color:var(--text-s);text-align:center;padding:8px;">No image</span><?php endif; ?>
              </div>
            ...
            <div style="display:flex;align-items:flex-start;gap:1rem;flex-wrap:wrap;">
              <div class="img-preview-wrap" id="img_preview_cert">
                <?php if ($uni['sample_certificate']): ?>
                  <img src="<?= e($uni['sample_certificate']) ?>" style="width:100%;height:100%;object-fit:cover;" alt="">
                <?php else: ?><span style="font-size:11px;color:var(--text-s);text-align:center;padding:8px;">No image</span><?php endif; ?>
              </div>
              <div style="flex:1;min-width:160px;">
          ...
          <div class="form-group full">
            ...
            <textarea name="key_advantages" class="form-control" rows="4"><?= e($old['key_advantages'] ?? '') ?></textarea>
          </div>
        </div>
      </div>
