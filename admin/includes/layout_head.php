...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .nav-item.active { background: rgba(79,110,247,0.15); color: var(--accent-h); }
  ...
  .user-meta .uname { font-size: 12.5px; font-weight: 600; color: var(--text); }
  .user-meta .urole { font-size: 11px; color: var(--text-s); text-transform: capitalize; }
  .btn-logout {
    display: flex; align-items: center; gap: 8px; width: 100%;
    padding: 0.55rem 0.65rem; border-radius: var(--radius-sm);
    border: none; background: none; color: var(--text-s);
  ...
  .topbar {
    ...
    padding: 0 1.75rem; background: var(--sidebar);
  ...
  .topbar-left p { font-size: 12px; color: var(--text-s); margin-top: 1px; }
  .topbar-right  { display: flex; align-items: center; gap: 10px; }

  .badge-date {
    background: var(--surface); border: 1px solid var(--border);
    border-radius: 6px; padding: 5px 10px; font-size: 12px; color: var(--text-m);
  }
  ...
  .btn-hamburger:hover { background: var(--surface-h); color: var(--text); }
  ...
  .btn-theme-toggle:hover { background: var(--surface-h); color: var(--text); border-color: var(--border-h); }
  body .icon-light        { display: none; }
  body .icon-dark         { display: flex; }
  body.light .icon-light  { display: flex; }
  ...
  .section-title {
    ...
    margin-bottom: 0.875rem; display: flex; align-items: center; gap: 8px;
  }
  ...
  .panel-header span { font-size: 13.5px; font-weight: 600; }
  .panel-header a    { font-size: 12px; color: var(--accent); text-decoration: none; }
  .panel-header a:hover { text-decoration: underline; }

  table { width: 100%; border-collapse: collapse; }
  table th {
    padding: 0.625rem 1.25rem; font-size: 11px; font-weight: 600;
    letter-spacing: 0.5px; text-transform: uppercase; color: var(--text-s);
    text-align: left; border-bottom: 1px solid var(--border);
    background: rgba(255,255,255,0.02);
  }
  table td {
    padding: 0.75rem 1.25rem; font-size: 13px;
    border-bottom: 1px solid var(--border); color: var(--text-m);
  }
  table tr:last-child td { border-bottom: none; }
  table tr:hover td { background: rgba(255,255,255,0.02); }
  table td .cell-name { color: var(--text); font-weight: 500; }
  ...
  body.light .alert-success { color: #15803d; }
  body.light .alert-error   { color: #b91c1c; }
  body.light .alert-warning { color: #92400e; }
  ...
  .form-group label { font-size: 13px; font-weight: 500; color: var(--text-m); }
  .form-group label span.req { color: var(--danger); margin-left: 2px; }

  .form-control {
    width: 100%; background: rgba(255,255,255,0.04);
    border: 1px solid var(--border); border-radius: var(--radius-sm);
    padding: 0.65rem 0.875rem; font-size: 13.5px;
    font-family: inherit; color: var(--text);
    transition: border-color 0.2s, box-shadow 0.2s; outline: none;
  }
  body.light .form-control { background: var(--bg); }
  .form-control::placeholder { color: var(--text-s); }
  .form-control:focus { border-color: var(--accent); box-shadow: 0 0 0 3px rgba(79,110,247,0.15); }
  .form-control:disabled { opacity: 0.5; cursor: not-allowed; }
  select.form-control { cursor: pointer; }
  textarea.form-control { resize: vertical; min-height: 90px; }

  .form-hint { font-size: 11.5px; color: var(--text-s); margin-top: 2px; }

  /* Checkbox / toggle */
  .check-group { display: flex; align-items: center; gap: 10px; padding: 0.5rem 0; }
  .check-group input[type=checkbox] {
    width: 16px; height: 16px; accent-color: var(--accent); cursor: pointer;
  }
  .check-group label { font-size: 13.5px; color: var(--text-m); cursor: pointer; margin: 0; }
  ...
  .check-pill {
    ...
    font-size: 12.5px; color: var(--text-m); cursor: pointer;
    transition: border-color 0.15s, color 0.15s, background 0.15s;
    user-select: none;
  }
  .check-pill input[type=checkbox] { display: none; }
  .check-pill:has(input:checked)   { border-color: var(--accent); color: var(--accent-h); background: rgba(79,110,247,0.1); }
  ...
  .img-preview-wrap {
    ...
    display: flex; align-items: center; justify-content: center;
    color: var(--text-s); font-size: 11px; background: var(--surface-h);
  }
  ...
  .btn {
    ...
    cursor: pointer; transition: opacity 0.2s, transform 0.15s, background 0.15s;
  ...
  .btn-secondary:hover { border-color: var(--border-h); background: var(--surface); }
  ...
  .page-header h3 { font-family: 'Space Grotesk', sans-serif; font-size: 20px; font-weight: 700; }
  .page-header p  { font-size: 13px; color: var(--text-s); margin-top: 3px; }
  ...
  .search-input-wrap svg { position: absolute; left: 0.75rem; top: 50%; transform: translateY(-50%); color: var(--text-s); pointer-events: none; }
  ...
  /* Empty state */
  .empty-state {
    text-align: center; padding: 3rem 1rem; color: var(--text-s);
  }
  ...
  .sidebar-close {
    ...
    border-radius: 6px; cursor: pointer; color: var(--text-m);
    transition: color 0.15s, background 0.15s;
  }
  .sidebar-close:hover { color: var(--text); background: rgba(255,255,255,0.1); }
  ...
  @media (max-width: 768px) {
    ...
    table th, table td { padding: 0.6rem 0.875rem; font-size: 12px; }
    .badge-date { display: none; }
    .page-header { flex-direction: column; align-items: flex-start; gap: 0.75rem; }
    .page-header .btn { align-self: flex-start; }
    .search-bar { flex-direction: column; align-items: stretch; }
    .search-bar select { width: 100% !important; min-width: unset !important; }
  }
